...
from airflow.providers.google.cloud.operators.cloud_storage_transfer_service import ( CloudDataTransferServiceCreateJobOperator, CloudDataTransferServiceUpdateJobOperator, CloudDataTransferServiceDeleteJobOperator, ) from airflow.providers.google.cloud.sensors.pubsub import PubSubPullSensor from google.cloud import pubsub_v1, storage from datetime import datetime import base64 import json

PROJECT_ID = "sandbox-corp-gdw-sfr-cdb8" DEST_BUCKET = "gdw2_sandbox-corp-gdw-sfr-cdb8" COMPLETION_SUB = f"projects/{PROJECT_ID}/subscriptions/sts-completion-sub" CDMNXT_TOPIC = f"projects/{PROJECT_ID}/topics/cdmnxt-trigger-topic"

def parse_apmf_event(**kwargs): ti = kwargs["ti"] raw = ti.xcom_pull(task_ids="wait_for_apmf_message") try: parsed = raw[0] message_data_b64 = parsed["message"]["data"] message_data = json.loads(base64.b64decode(message_data_b64).decode("utf-8")) file_events = message_data["file_events"] except Exception as e: raise ValueError(f"[ERROR] Failed to parse message: {raw}, error: {e}")
...
def create_update_run_tasks(task_group_name, idx): def get_job_body(ti, **kwargs): return ti.xcom_pull(task_ids="build_transfer_payload", key=f"transfer_job{idx}")
...
create_job = CloudDataTransferServiceCreateJobOperator(
    ...
    body=_get_job_body(ti=None),  # will patch below
...
wait_for_apmf_message = PubSubPullSensor(
    ...
    for i in range(2):
        ...
        build_transfer_payload_task >> tasks[0] >> tasks[1] >> tasks[2] >> tasks[3] >> tasks[4]
