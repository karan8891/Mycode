...
def create_sts_job(conf, job_id, delete_flag): prefix = conf.get("cdm_file_prefix_pattern", "") return { "description": job_id, "project_id": PROJECT_ID, "transfer_spec": { "gcs_data_source": {"bucket_name": conf["cdm_source_bucket"]}, "gcs_data_sink": {"bucket_name": DEST_BUCKET}, "object_conditions": {"include_prefixes": [prefix]}, "transfer_options": {"delete_objects_from_source_after_transfer": delete_flag}, }, "notification_config": { "pubsub_topic": f"projects/{PROJECT_ID}/topics/sts-completion-topic", "event_types": ["TRANSFER_OPERATION_SUCCESS"], "payload_format": "JSON", }, }

def build_transfer_payload(**kwargs): ti = kwargs["ti"] grouped = ti.xcom_pull(key="grouped_events", task_ids="parse_apmf_event") outputs = []
...
for delete_flag, events in grouped.items():
    ...
    outputs.append(job)
    ti.xcom_push(key=f"transfer_job_{len(outputs) - 1}", value=job)
    ti.xcom_push(key=f"transfer_body_{len(outputs) - 1}", value=create_sts_job(job["conf"], job_id, delete_flag))

ti.xcom_push(key="transfer_jobs", value=outputs)

def notify_cdmnxt(**kwargs): transfers = kwargs["ti"].xcom_pull(key="transfer_jobs", task_ids="build_transfer_payload") publisher = pubsub_v1.PublisherClient()
...
def create_update_run_tasks(task_group_name, idx): return [ CloudDataTransferServiceCreateJobOperator( task_id=f"create_sts_job_{idx}", body="{{ task_instance.xcom_pull(task_ids='build_transfer_payload', key='transfer_body_" + str(idx) + "') }}", project_id=PROJECT_ID, ), CloudDataTransferServiceUpdateJobOperator( task_id=f"run_sts_job_{idx}", job_name="{{ task_instance.xcom_pull(task_ids='create_sts_job_" + str(idx) + "')['name'] }}", body="{}", project_id=PROJECT_ID, ), PubSubPullSensor( task_id=f"wait_for_completion_{idx}", project_id=PROJECT_ID, subscription=COMPLETION_SUB, ack_messages=True, max_messages=5, timeout=600, ), CloudDataTransferServiceDeleteJobOperator( task_id=f"delete_sts_job_{idx}", job_name="{{ task_instance.xcom_pull(task_ids='create_sts_job_" + str(idx) + "')['name'] }}", project_id=PROJECT_ID, ), ]

def build_dag(): with models.DAG( dag_id="Five9_to_CDM_Test", start_date=days_ago(1), schedule_interval=None, catchup=False, tags=["apmf", "gdw", "sts", "event-driven"], ) as dag:
