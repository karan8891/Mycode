    ...
    with models.DAG(
        ...
        for i in range(2):
            ...
            create_job = CloudDataTransferServiceCreateJobOperator(
                ...
                body="{{ task_instance.xcom_pull(task_ids='prepare_body_" + str(i) + "') }}",
                project_id=PROJECT_ID,
            )

            run_job = CloudDataTransferServiceUpdateJobOperator(
                task_id=f"run_sts_job_{i}",
                job_name="{{ task_instance.xcom_pull(task_ids='create_sts_job_" + str(i) + "')['name'] }}",
                body={},
                project_id=PROJECT_ID,
            )
            ...
            build_transfer_payload_task >> prepare_body >> create_job >> run_job >> wait_completion >> delete_job >> notify_cdmnxt_task
