...
with DAG(
    ...
    def resolve_sts_body(**kwargs):
        ti = kwargs["ti"]
        return ti.xcom_pull(task_ids="build_sts_job", key="sts_body")

    def resolve_sts_job_name(**kwargs):
        ti = kwargs["ti"]
        return ti.xcom_pull(task_ids="build_sts_job", key="job_name")
    ...
    resolve_body = PythonOperator(
        task_id="resolve_sts_body",
        python_callable=resolve_sts_body,
    )

    create_sts_job = CloudDataTransferServiceCreateJobOperator(
        task_id="create_sts_job",
        body="{{ task_instance.xcom_pull(task_ids='resolve_sts_body') }}",
    )
