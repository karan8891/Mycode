...
with DAG(
    dag_id="gdw_sts_event_v9_final_fixed",
    ...
    def pull_cf_message(**kwargs):
        ...
        for msg in response.received_messages:
            msg_data = msg.message.data.decode("utf-8")
            kwargs["ti"].xcom_push(key="cf_msg", value=msg_data)
            ack_ids.append(msg.ack_id)
        if ack_ids:
            subscriber.acknowledge(subscription=subscription_path, ack_ids=ack_ids)

    def build_sts_body(**kwargs):
        msg_str = kwargs["ti"].xcom_pull(key="cf_msg")
        cf_message = json.loads(msg_str)
        ...
        job_name = "transfer_job_" + uuid.uuid4().hex
    ...
    def create_toc_file(**kwargs):
        ...
        toc_content = "\n".join(filenames)
        toc_blob.upload_from_string(toc_content)
        kwargs["ti"].xcom_push(key="toc_filename", value=toc_filename)
        kwargs["ti"].xcom_push(key="toc_file_content", value=toc_content)

    def notify(**kwargs):
        publisher = pubsub_v1.PublisherClient()
        topic_path = publisher.topic_path(PROJECT_ID, CDMNXT_TOPIC)
        toc_name = kwargs["ti"].xcom_pull(key="toc_filename", default="unknown_toc.txt")
        content = kwargs["ti"].xcom_pull(key="toc_file_content", default="(empty)")
        try:
    ...
    t3 = CloudDataTransferServiceCreateJobOperator(
        ...
        project_id=PROJECT_ID,
        body=lambda **kwargs: kwargs["ti"].xcom_pull(task_ids="build_sts_body", key="sts_body")
    )
