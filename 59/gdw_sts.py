...
def create_update_run_tasks(task_group_name, idx):
    return [
        CloudDataTransferServiceCreateJobOperator(
            ...
            body=f"{{{{ task_instance.xcom_pull(task_ids='build_transfer_payload', key='transfer_job_{idx}') }}}}",
            project_id=PROJECT_ID,
        ),
        CloudDataTransferServiceUpdateJobOperator(
            task_id=f"run_sts_job_{idx}",
            job_name=f"{{{{ task_instance.xcom_pull(task_ids='create_sts_job_{idx}')['name'] }}}}",
            project_id=PROJECT_ID,
        ),
        PubSubPullSensor(
