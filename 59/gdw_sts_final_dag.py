...
def create_sts_job(conf, job_id, delete_flag):
    return {
        ...
        "transfer_spec": {
            ...
            "object_conditions": {"include_prefixes": conf.get("cdm_file_prefix_pattern", "")},
...
def notify_cdmnxt(**kwargs):
    ...
    for t in transfers:
        msg = {
            "toc_file": t["toc_file"],
            "source_bucket": t["conf"].get("cdm_source_bucket"),
            "source_project": t["conf"].get("cdm_source_project_id"),
    ...
    def create_update_run_tasks(task_group_name, idx):
        return [
            ...
            CloudDataTransferServiceUpdateJobOperator(
                ...
                project_id=PROJECT_ID,
                body="{{ task_instance.xcom_pull(task_ids='create_sts_job_" + str(idx) + "')['name'] }}",
            ),
            PubSubPullSensor(
