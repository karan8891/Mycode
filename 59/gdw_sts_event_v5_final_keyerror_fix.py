...
with DAG(
    ...
    def build_sts_body(**kwargs):
        import time
        cf_message_str = kwargs["ti"].xcom_pull(key="cf_msg")
        cf_message = json.loads(cf_message_str)
...
".join(filenames))
    ...
    t3 = CloudDataTransferServiceCreateJobOperator(
        ...
        project_id=PROJECT_ID,
        body="{{ ti.xcom_pull(task_ids='build_sts_body', key='sts_body') | fromjson }}"
    )
