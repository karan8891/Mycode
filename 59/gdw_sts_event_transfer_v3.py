...
TOPIC_NAME = "cdmnxt-trigger-topic"
...
with DAG(
    ...
    def parse_cf_message(**kwargs):
        ...
        for msg in messages:
            try:
                ...
                if not raw_data:
                    print("[WARNING] Skipping message: empty 'data' field")
                    continue
                decoded = base64.b64decode(raw_data).decode("utf-8").strip()
                if not decoded:
                    print("[WARNING] Skipping message: base64 decoded data is empty")
                    continue
    ...
    def create_sts_body(**kwargs):
        ...
        destination_bucket = "gdw-destination-bucket"
        ...
        body = {
            ...
            "notificationConfig": {
                "pubsubTopic": f"projects/{PROJECT_ID}/topics/sts-completion-topic",
    ...
    create_sts_job = CloudDataTransferServiceCreateJobOperator(
        ...
        body="{{ task_instance.xcom_pull(task_ids='build_sts_job', key='sts_body') }}",
    )
    ...
    delete_sts_job = CloudDataTransferServiceDeleteJobOperator(
        ...
        job_name="{{ task_instance.xcom_pull(task_ids='build_sts_job', key='job_name') }}",
        project_id=PROJECT_ID,
    )
